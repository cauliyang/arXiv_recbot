...
def train_model(data):
    ...
    class_weights = torch.tensor(
        [(y_train == i).sum().item() for i in range(len(set(y_train)))]
    )
    ...
    def weighted_kl_loss(pred, target, class_weights):
        ...
        kl_loss = nn.KLDivLoss(reduction="none")(
            pred.log_softmax(dim=1), smooth_targets
        )
    ...
    def criterion(pred, target):
        return weighted_kl_loss(pred, target, class_weights)
...
async def main():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY, message_id INTEGER, text TEXT, preference INTEGER)"
    )
    ...
    async for message in client.iter_messages(entity, limit=limit, reverse=False):
        ...
        if message.reply_to_msg_id:
            ...
            and message.text.startswith("https://arxiv.org")
        ):
        ...
        if entry is not None:
            ...
            if cursor.fetchone()[0] == 0:
                cursor.execute(
                    "INSERT INTO messages (message_id, text, preference) VALUES (?, ?, ?)",
                    (entry["message_id"], entry["text"], entry["preference"]),
                )
                conn.commit()
            else:
                break
